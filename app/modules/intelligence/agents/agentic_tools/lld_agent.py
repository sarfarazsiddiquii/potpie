...
class DesignStep(BaseModel):
    ...
    relevant_files: List[str] = Field(..., description="List of relevant files for this step")
    code_changes: Dict[str, str] = Field(..., description="Proposed code changes for each file")

class LowLevelDesignPlan(BaseModel):
    feature_name: str = Field(..., description="Name of the feature being implemented")
    overview: str = Field(..., description="High-level overview of the implementation plan")
    design_steps: List[DesignStep] = Field(..., description="Detailed steps for implementing the feature")
    potential_challenges: List[str] = Field(..., description="Potential challenges or considerations")
...
class LowLevelDesignAgent:
    def __init__(self, sql_db, llm, user_id):
        ...
        self.ask_knowledge_graph_queries = get_ask_knowledge_graph_queries_tool(sql_db, user_id)
    ...
    async def create_tasks(self, functional_requirements: str, project_id: str, codebase_analyst, design_planner):
    ...
    async def run(self, functional_requirements: str, project_id: str) -> LowLevelDesignPlan:
        os.environ["OPENAI_API_KEY"] = self.openai_api_key

        codebase_analyst, design_planner = await self.create_agents()
        tasks = await self.create_tasks(functional_requirements, project_id, codebase_analyst, design_planner)
