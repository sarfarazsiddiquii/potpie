...
class MessageService:
        ...
        try:
            if (message_type == MessageType.HUMAN and sender_id is None) or \
               (message_type in {MessageType.AI_GENERATED, MessageType.SYSTEM_GENERATED} and sender_id is not None):
                raise InvalidMessageError("Invalid sender_id for the given message_type.")
            ...
            logger.info(f"Created new message with ID: {message_id} for conversation: {conversation_id}")
            return new_message
            ...
            logger.error(f"Database integrity error in create_message: {e}", exc_info=True)
            raise MessageServiceError("Failed to create message due to a database integrity error") from e

        except Exception as e:
            logger.error(f"Unexpected error in create_message: {e}", exc_info=True)
            raise MessageServiceError("An unexpected error occurred while creating the message") from e
    ...
    async def mark_message_archived(self, message_id: str) -> None:
        try:
            ...
            raise MessageServiceError(f"Failed to archive message {message_id} due to a database error") from e
        except Exception as e:
            logger.error(f"Unexpected error in mark_message_archived: {e}", exc_info=True)
            raise MessageServiceError(f"An unexpected error occurred while archiving message {message_id}") from e
